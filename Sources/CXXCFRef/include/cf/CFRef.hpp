...
template <typename T> inline auto CFRef<T>::adopt(T _Nullable object) noexcept -> CFRef {
    return CFRef(object);
}
...
template <typename T> inline CFRef<T>::~CFRef() noexcept {
    reset();
}

// MARK: Core Foundation Object Management

template <typename T> inline CFRef<T>::operator bool() const noexcept {
    return object_ != nullptr;
}

template <typename T> inline CFRef<T>::operator T() const noexcept {
    return object_;
}
...
template <typename T> inline T _Nullable CFRef<T>::get() const& noexcept {
    return object_;
}
...
template <typename T> inline void CFRef<T>::swap(CFRef& other) noexcept {
    std::swap(object_, other.object_);
}

template <typename T> inline T _Nullable CFRef<T>::leak() noexcept {
